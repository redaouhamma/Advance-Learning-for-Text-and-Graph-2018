...
def unweighted_k_core(g):
    ...
    while len(gg.vs)>0:
        ### fill the gaps ###
        temp =[vertice for vertice in gg.vs if gg.strength(vertice)< i +1 ]

        while temp :
            for vertex in temp :
                cores_g[ vertex['name'] ] = i
            gg.delete_vertices(temp)
            temp = [ vertice for vertice in gg.vs if gg.strength(vertice)< i +1 ]
        i = i + 1
    return cores_g
    ...
    while len(heap_g) > 0:
        ...
        # iterate over neighbors of top element
            for i, name_n in enumerate(neighbors_top):
                ...
                max_n = max( cores_g[top], gg.strength( index_n , weights = gg.es['weight']  ) )
