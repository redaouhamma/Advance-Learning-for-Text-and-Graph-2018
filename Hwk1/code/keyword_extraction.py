...
for counter,abstract in enumerate(abstracts_cleaned):
    ...
    # create graph-of-words
    g = terms_to_graph(abstract, w=4)
    # decompose weighted graph-of-words
    core_numbers = weighted_core_dec(g)
    # retain main core as keywords
    max_c_n = max(core_numbers.values())
    #take the 33% most important words
    keywords = [kwd for kwd,c_n in core_numbers.iteritems() if c_n >= 0.66 * max_c_n]
    # save results
...
for counter,vector in enumerate(vectors_list):
    ...
    nonzero = [tuple for tuple in terms_weights if tuple[0] in abstracts_cleaned[counter ] ]
...
for counter,abstract in enumerate(abstracts_cleaned):
    ...
    g = terms_to_graph(abstract, w=4)
    # PageRank graph-of-words
    ranking = g.pagerank()
    word_rank = zip(g.vs['name'], ranking)
    word_rank = sorted(word_rank, key=operator.itemgetter(1), reverse=True)
    numb_to_retain = int(len( word_rank )* my_percentage)
    keywords = [tuple[0] for tuple in word_rank[:numb_to_retain]]
...
print "We canno't compare all approaches globaly since every one of them is better than the other for one of the three metrics \n However, GOW doesn't take into account the asymetry of links so it is a clear disadvantage"
