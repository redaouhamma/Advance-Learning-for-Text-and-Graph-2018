...
all_terms = [ term for doc in terms_by_doc for term in doc] ### fill the gap ### hint: flatten 'terms_by_doc' (you may use a list comprehension)
...
for counter,unique_term in enumerate(list(idf.keys())):
    ...
    df =  sum( unique_term in doc_set for doc_set in terms_by_doc_sets )### fill the gap ### 
    ...
    # idf
    idf[unique_term] = math.log10( ( n_doc + 1 ) / df ) ### fill the gap ### hint: use math.log10 and refer to the beginning of Section 2 in the handout
    if counter % 1e3 == 0:
...
c_g = terms_to_graph(terms_by_doc, 3, overspanning = False )  ### fill the gap ### hint: use the terms_to_graph function with the proper arguments

# sanity check (should return True)
assert(len(all_unique_terms) == len(c_g.vs))
...
collection_degrees = dict( zip( c_g.vs[ 'name' ], [ vertice.strength() for vertice in c_g.vs ] ) ) ### fill the gap ### hint: build a dict where the keys are the names of the nodes in the collection graph and the values are their unweighted degrees
...
for i, graph in enumerate(all_graphs):
    ...
    for term in list(set(terms_in_doc)):
        ...
        idf_term = idf[term]
        b = 0.2
        denominator =  1. - b + b * doc_len / avg_len  ### fill the gap ### hint: refer to the TF equation in the handout
